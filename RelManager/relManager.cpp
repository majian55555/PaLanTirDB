...
const Status InsertOperator::insertLRecord(const Record & rec, RID& outRid)
{
	...
	do
	{
		...
		{
			if((status=tmpPage->insertPureData((void*)((unsigned long)rec.getRealData()+offset),PAGESIZE-DPFIXED))!=OK)
				return status;
			remainLen-=(PAGESIZE-DPFIXED);
...
const Status InsertOperator::updateRecord(const Record & rec)
{
	...
	if(status==NOSPACE)//we want to update a large record
	{
		...
		this->dirtyCDP=true;
		return this->insertRecord(rec,this->curRec);
	}
	ASSERT(status==OK);
	int curIdx=this->curRHP->getCurIdx(this->curDataPageNo);
	ASSERT(curIdx!=-1);
                                     ...
                                     const Operator op_)
{
	...
	if((attrLen_<1)||(type_ !=CHAR&&type_!=VCHAR&&type_!=INTEGER&&type_!=FLOAT&&type_!=DATE&&type_!=DATETIME&&type_!=BOOLEAN)||
      (type_==INTEGER&&attrLen_!=SIZEOFINT||type_==FLOAT&&attrLen_!=SIZEOFFLOAT||type_==DATE&&attrLen_!=SIZEOFDATE||type_==DATETIME&&attrLen_!=SIZEOFDATETIME||type_==BOOLEAN&&attrLen_!=SIZEOFBOOL)||
      (op_ !=LT&&op_!=LTE&&op_!=EQ&&op_!=GTE&&op_!=GT&&op_!=NE)||
	  (type_==BLOB&&type_==CLOB)||(type_==CHAR&&attrLen_>MAXOFCHAR||type_==VCHAR&&attrLen_>MAXOFCHAR))
    {
        return BADSCANPARM;
    }
