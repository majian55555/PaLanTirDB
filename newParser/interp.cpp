...
void interp(NODE *n)
{
	...
	switch(n->kind)
	{
		...
		// First check if the result relation is specified
			...
			if (temp == NULL)
			{
				...
				if (status == RELNOTFOUND)
				{
					...
					status=relCat->RelCat_createRel(resultName,nattrs,createAttrInfo);
			...
			else if (temp->kind == N_SELECT)
			{
				...
				errval = QU_Select(resultName,nattrs,attrList,&attr1,(Operator)temp->u.SELECT.op,tmpValue);
				delete [] tmpValue;
			...
			// make the call to QU_Join
				errval = QU_Join(resultName,nattrs,attrList,&attr1,(Operator)temp->u.JOIN.op,&attr2);
				if (errval != OK)
					error.print((Status)errval);
			}
			if (resultName == std::string( "Tmp_Ubase_Result"))
			{
				// Print the contents of the result relation and destroy it
				status = PLT_Print(db->getDBName(),resultName);
				if (status != OK)
					error.print(status);
				status = relCat->RelCat_destroyRel(resultName);
				if (status != OK)
					error.print(status);
			}
			break;
		...
		case N_DELETE:
			...
			if (attrname)
				errval = QU_Delete(n -> u.DELETE.relname,attrname,(Operator)op,(Datatype)type,(char *)value);
			else
				errval = QU_Delete(n -> u.DELETE.relname,"",(Operator)op,(Datatype)type,(char *)value);
			delete [] value;
			if (errval != OK)
				error.print((Status)errval);
			break;
		case N_UPDATE:
			...
			if (attrname)
				errval=QU_Update(n->u.UPDATE.relname,nattrs,attrList,attrname,(Operator)op,(Datatype)type,(char*)value);
			else
				errval=QU_Update(n->u.UPDATE.relname,nattrs,attrList,"",(Operator)op,(Datatype)type,(char*)value);
			delete [] value;
			if (errval != OK)
				error.print((Status)errval);
			break;
...
static int mk_attrnames(NODE *list, char *attrnames[], char *relname)
{
	...
	for(i = 0; list != NULL && i < MAXATTRS; ++i, list = list->u.LIST.next)
	{
		...
		if (list->u.LIST.self->u.QUALATTR.attrname[0]=='*')
		{
			...
			{
				attrnames[j]=const_cast<char*>(attrs[k].getAttrName().c_str());
...
static int mk_attr_descrs(NODE *list, ATTR_DESCR attr_descrs[])
{
	...
	for(i = 0; list != NULL && i < MAXATTRS; ++i, list = list->u.LIST.next)
	{
		...
		/*errval = parse_format_string(attr->u.ATTRTYPE.type, &type, &len, &canbenull);
		if (errval != E_OK)
			return errval;*/
		// add it to the list
