...
////////////////////////////////////////////////////////////////////////////////////////////
//获取关系（表）数据页页号
/*const int DataPageRec::getPageNo() const
{
	return this->pageNo;
}
//获取关系（表）数据页的空闲空间
const int DataPageRec::getFreeSpace() const
{
	return this->freeSpace;
}
//置关系（表）数据页页号
const Status DataPageRec::setPageNo(int pageNo)
{
	if(pageNo<-1)
		return BADPAGENO;
	this->pageNo=pageNo;
...
//置关系（表）数据页的空闲空间
const Status DataPageRec::setFreeSpace(int freeSpace)
{
	if(freeSpace<0)
		return BADFREESPACE;
...
void DBHeadPage::init()
{
	...
	this->nextFreePage=0;
	this->pageCnt=1;
	strcpy(this->DBsignature,"MJ's DataBase");
...
{	
	if(this->file.seekp(pageNo*sizeof(Page))==NULL)
		return UNIXERR;
	if(this->file.read((char*)pagePtr,sizeof(Page))==NULL)
		return UNIXERR;
  return OK;
}
...
const Status DB::intwrite(const unsigned long pageNo, const Page* pagePtr)
{
	if(this->file.seekp(pageNo*sizeof(Page))==NULL)
		return UNIXERR;
	if(this->file.write((char*)pagePtr, sizeof(Page))==NULL)
		return UNIXERR;
	return OK;
}
