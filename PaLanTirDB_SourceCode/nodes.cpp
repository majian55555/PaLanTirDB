...
NODE *update_node(char *relname, NODE *attrlist, NODE *qual)
{
  ...
  n->u.UPDATE.attrlist = attrlist;
  n->u.UPDATE.qual = qual;
  return n;
}
...
{
  NODE *n = newnode(N_CREATEREL);
    
  n->u.CREATEREL.relname = relname;
  n->u.CREATEREL.attrlist = attrlist;
  n->u.CREATEREL.primattr = primattr;
  return n;
}
...
{
  NODE *n = newnode(N_DESTROYREL);
  
  n->u.DESTROYREL.relname = relname;
  return n;
}
...
{
  NODE *n = newnode(N_DESTROYDB);
  
  n->u.DESTROYDB.dbname = dbname;
  return n;
}
NODE *openDB_node(char *dbname)
{
  NODE *n = newnode(N_OPENDB);
  
  n->u.OPENDB.dbname = dbname;
  return n;
}
NODE *closeDB_node(char *dbname)
{
  NODE *n = newnode(N_CLOSEDB);
  
  n->u.CLOSEDB.dbname = dbname;
  return n;
}
//
...
NODE *attrtype_node(char *attrname, int type ,int size ,int canbenull)
{
  ...
  switch(type)
  {
	...
	case 1 :
		...
		break;
	case 2 :
		n->u.ATTRTYPE.size=SIZEOFINT;
		break;
	case 3 :
		n->u.ATTRTYPE.size=SIZEOFFLOAT;
		break;
	case 4 :
		n->u.ATTRTYPE.size=SIZEOFDATE;
		break;
	case 5 :
		n->u.ATTRTYPE.size=SIZEOFDATETIME;
		break;
	case 6 :
		n->u.ATTRTYPE.size=SIZEOFBOOL;
		break;
	case 7 :
		n->u.ATTRTYPE.size=0;
		break;
	case 8 :
		...
		break;
	default:
		printf("/nBad data type in create table./n");
  }
  n->u.ATTRTYPE.canbenull=canbenull;
  return n;
}
